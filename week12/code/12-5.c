...
int main(void)
{
	...
	else if(pid == 0)
	{
		...
		while(loop++ < 10)
		{
			...
			printf("[child-A] pid=%d write %d bytes to pipe.\n",getpid(),ret);
		}
		exit(0);
	}
	...
	else if(pid == 0)
	{
		...
		while(loop++ < 10)
		{
			...
			printf("[child-B] pid=%d write %d bytes to pipe.\n",getpid(),ret);
		}
		exit(0);
	}
	...
	else if(pid == 0)
	{
		...
		while(loop++ < 10)
		{
			...
			printf("[child-C] pid=%d write %d bytes to pipe.\n",getpid(),ret);
		}
		exit(0);
	}
	...
	close(fd[1]);
	sleep(2);
	int f = open("./test.txt", O_WRONLY|O_CREAT|O_TRUNC, 0644);
	char buf[1024*4] = {0};
	int n = 1;
	while(1)
	{
